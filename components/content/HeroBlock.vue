<template>
  <section ref="heroSection" class="relative flex flex-col items-center justify-center min-h-screen px-3 prose dark:prose-invert">
    ...
    <a href="#about-me" class=" not-prose my-2 px-6 py-2 rounded-full font-medium transition-colors bg-primary-500 text-white hover:bg-primary-600 hover:text-white">
      See More About Me
    </a>
    </div>
  </section>
...
function generateNetwork(ctx, width, height) {
  ...
  function addNode() {
    if (nodes.length === 0 || (nodes.length < maxNodes && frameCount % 30 === 0)) {
      let newNode;
      if (nodes.length === 0) {
        newNode = {
          x: width / 2,
